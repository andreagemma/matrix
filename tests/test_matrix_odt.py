...
def test_initialization_label_access_and_missing_timestamp() -> None:
    ...
    missing_timestamp = matrix[99]
    if isinstance(missing_timestamp, MatrixOD):
        assert missing_timestamp.sum() == 0
    else:
        assert missing_timestamp == 0
...
def test_csv_and_dataframe_roundtrip(tmp_path: Path) -> None:
    matrix = make_matrix_odt()
    ...
    assert isinstance(loaded_t0, MatrixOD)
    assert isinstance(loaded_t1, MatrixOD)
    assert isinstance(matrix_t0, MatrixOD)
    assert isinstance(matrix_t1, MatrixOD)

    np.testing.assert_array_equal(loaded_t0.mat, matrix_t0.mat)
    np.testing.assert_array_equal(loaded_t1.mat, matrix_t1.mat)
